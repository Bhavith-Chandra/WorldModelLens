...
class PhysicalVariableAnalyzer:
    ...
    def discover_subspaces(
        ...
        Args:
            ...
            method: 'pca', 'ica', 'nmf', or 'gradient'
            n_components: Number of directions to extract (only for unsupervised methods)
            probes: Trained probes dictionary (required for 'gradient')
        """
        ...
        elif method_lower == "gradient":
            if probes is None:
                raise ValueError("Probes dictionary must be provided when using 'gradient' method.")
            return discover_subspaces_gradient(probes, activations), None
        else:
            raise ValueError(f"Unknown subspace discovery method: {method}. Choose 'pca', 'ica', 'nmf', or 'gradient'.")
