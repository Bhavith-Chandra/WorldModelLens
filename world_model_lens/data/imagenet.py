"""Deterministic ImageNet subset selection and I-JEPA preprocessing."""
...
def load_imagenet_subset(
    ...
    """Return a balanced, deterministic subset manifest from an ImageNet tree."""
    ...
    per_class, remainder = divmod(num_samples, num_classes)
    max_per_class = per_class + int(remainder > 0)
    ...
    if len(available) < num_classes:
        raise ValueError(
            ...
            f"with at least {max_per_class} images each"
        )
    ...
    for label, (class_name, paths) in enumerate(chosen):
        class_count = per_class + int(label < remainder)
        for path in rng.sample(paths, class_count):
...
def load_imagenet_image(path: str | Path, image_size: int = 224) -> torch.Tensor:
    """Load RGB, square-resize with bicubic interpolation, normalize, and batch."""
